...
export const makeClient = (): ApolloClient<InMemoryCache> => {
    ...
    const getWsUrl = () => {
        ...
        if (process.env.NEXT_PUBLIC_GRAPHQL_ENDPOINT) {
            ...
            return process.env.NEXT_PUBLIC_GRAPHQL_ENDPOINT.replace(/^http:/, 'ws:').replace(
                /^https:/,
                'wss:'
            );
        }
            ...
            ? new GraphQLWsLink(
                  createClient({
                      ...
                      retryWait: async (retries) => {
                          ...
                          console.log(
                              `[WebSocket] Retrying in ${waitTime}ms (attempt ${retries + 1})`
                          );
