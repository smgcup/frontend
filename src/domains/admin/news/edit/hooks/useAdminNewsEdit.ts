...
export const useAdminNewsEdit = (newsId: string) => {
	...
	const {
		...
		loading: newsLoading,
		error: newsError,
	} = useQuery<GetNewsByIdQuery, GetNewsByIdQueryVariables>(
		GetNewsByIdDocument,
		{
	...
	const [updateNewsMutation, { loading: updateLoading, error: updateError }] =
		useMutation<UpdateNewsMutation, UpdateNewsMutationVariables>(
			UpdateNewsDocument
		);
